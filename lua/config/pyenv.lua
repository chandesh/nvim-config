...
local required_packages = {
  "ruff", -- for linting
  -- Note: Using Pyright instead of pylsp to avoid PathD errors
}
...
function M.is_package_installed(package_name)
  ...
  local cmd =
    string.format("%s -c 'import pkg_resources; pkg_resources.require(\"%s\")'", python_executable, package_name)
...
function M.ensure_lsp_packages()
  ...
  if #missing_packages > 0 then
    vim.notify(string.format("Installing Python LSP packages in environment '%s'...", env_name), vim.log.levels.INFO)

    for _, package in ipairs(missing_packages) do
      if M.install_package(package) then
        vim.notify(string.format("Installed %s in environment '%s'", package, env_name), vim.log.levels.INFO)
      else
        vim.notify(string.format("Failed to install %s in environment '%s'", package, env_name), vim.log.levels.ERROR)
      end
    end
  end
end
